...
1. `analyzeGarment(spec, model)` — one pass over the map, cached (LRU 6). Yields: panel id per pixel, garment + per-panel bbox, per-panel centroid + grain angle (PCA or drawn axis), curved limb axes (drawn strokes), fan frames (blue corner dots), trim luminance ref `Lref` (p90 of trim), fabric white `Fref` (p95 per channel), and for model views a masked low-pass of the trim luminance `tlo`.
2. `compose(G, pat, ratio, trim)` — raster the pattern to `ratio × mockW`; build the fold-displacement field (box blur of the mock luminance, `DISP_BLUR` 8 px) over the garment box; for each fabric pixel, shift the sample point along the luminance gradient (up to `DISP_AMP` 12 px, print compressed on fold flanks), map it through the panel's local frame (fan / curved axis / rotation), bilinear-sample the tile, multiply by `mock/Fref`, keep the mock for the non-fabric share of an edge pixel (coverage `cov` measured in `analyzeGarment` from a smoothed raster of the map: R / (id×20) where the 3×3 neighbourhood touches id 0; inside the fabric coverage is exactly 1, so no halo next to a white cord or a hand); recolour trim; downscale by `SS`. Returns a canvas.
...
- **Green + blue `≥ 250`** = a perspective corner dot, read only by `pattern-mockup-v2.html` (homography); this engine ignores it.
...
- **Flatlay**: `f = lum/Lref`, hard-clamped between `TRIM_FLOOR` (0.65) and `TRIM_CEIL` (1.15).
- **Model view**: soft-knee (`TRIM_KNEE` 0.86 / `TRIM_ROLL` 0.22 / `TRIM_MCEIL` 1.00) and soft-floor (`TRIM_MFLOOR` 0.50) act on the **low-pass** `tlo` only (kills specular gloss + lit-collar glare); weave/stitching/fold detail (`lum/lo`) rides on top untouched.

### Key constants

`OUT_AREA` ~1900px wide · `SS` 1.3 supersample · `OFFX`/`OFFY` per-panel tile phase (seam decorrelation) · `TRIM_BLUR` 0.006 low-pass radius · `DISP_AMP` 12 / `DISP_BLUR` 8 / `DISP_GREF` 3 fold displacement · `AX_RMIN` 2 curved-axis fold guard. Edge softness has no constant: it is the map's own sub-pixel coverage.

## Pipeline vs Photoshop

Every step in run order, with the closest Photoshop operation. Three steps have no direct equivalent (3, 8, 12), which is why the engine cannot be rebuilt as an action / PSD.
...
| 2 | Read the map PNG: R/20 = panel id, G ≥ 250 = grain stroke, B ≥ 250 = fan corner dot | One layer mask / selection per panel (Color Range on the map); strokes and dots = Warp / Puppet Warp anchors | PS has no 3-channel map: every mask is drawn by hand |
...
| 5 | Four blue dots → fan (x = arc length, y = radius) | Filter > Distort > Polar Coordinates, or Warp > Arc | Skirts |
| 6 | `Lref` = p90 trim luminance, `Fref` = p95 fabric white per channel | Levels: "Set White Point" eyedropper on the brightest fabric | Normalises the mock to white before Multiply, so a warm/pink photo does not tint the print |
...
| 12 | Soft edge: blend the mock in by `cov` read from the smoothed map raster, only where a pixel touches background / skin | Anti-aliased mask, **no** Feather | A feathered / blurred mask caused a bright rim next to a white cord; dropped |
| 13 | Trim, flatlay: chosen colour × `lum/Lref`, clamped 0.65–1.15 | Solid Color fill layer, Multiply, clipped to the trim mask, plus Levels to limit the range | — |
| 14 | Trim, model: soft knee on the **low-pass** `tlo` (blur ~0.6 % of width), high-frequency `lum/lo` added back untouched | Frequency separation: Gaussian Blur for the low, Curves compressing highlights / shadows on the low, high layer in Linear Light | Kills specular gloss and lit-collar glare while keeping the weave |
| 15 | Downscale to `W/SS` | Image Size down, Bicubic | — |
